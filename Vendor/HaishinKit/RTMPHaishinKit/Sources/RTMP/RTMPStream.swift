...
public actor RTMPStream {
    ...
    private func append(_ message: RTMPAudioMessage, type: RTMPChunkType) {
        ...
        guard message.payload.count >= 2, message.codec.isSupported else {
            return
        }
        ...
        case RTMPAACPacketType.raw.rawValue:
            ...
            if let packet = message.makeAudioBuffer(format: audioFormat) {
                let timestamp = audioTimestamp.value
                Task { await incoming.append(packet, when: timestamp) }
            }
        default:
            break
        }
    ...
    private func append(_ message: RTMPVideoMessage, type: RTMPChunkType) {
        ...
        let timestamp = videoTimestamp.value
        ...
        if message.isExHeader {
            ...
            case RTMPVideoPacketType.codedFrames.rawValue:
                Task { await incoming.append(message, presentationTimeStamp: timestamp, formatDesciption: format) }
...
extension RTMPStream: _Stream {
    ...
    public func append(_ sampleBuffer: CMSampleBuffer) {
        ...
        case .video:
            if sampleBuffer.formatDescription?.isCompressed == true {
                ...
                if readyState != .playing { outgoing.append(sampleBuffer) }
    ...
    public func append(_ audioBuffer: AVAudioBuffer, when: AVAudioTime) {
        ...
        default:
            if readyState != .playing { outgoing.append(audioBuffer, when: when) }
