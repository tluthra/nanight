...
struct RTMPAudioMessage: RTMPMessage {
    ...
    func makeAudioBuffer(format: AVAudioFormat?) -> AVAudioCompressedBuffer? {
        guard let format, codec.isSupported, payload.count > codec.headerSize else { return nil }
        let buffer = AVAudioCompressedBuffer(format: format, packetCapacity: 1, maximumPacketSize: payload.count - codec.headerSize)
        copyMemory(buffer)
        return buffer
    }
    ...
    func copyMemory(_ audioBuffer: AVAudioCompressedBuffer?) {
        payload.withUnsafeBytes { (buffer: UnsafeRawBufferPointer) in
            ...
            guard byteCount > 0, byteCount <= audioBuffer.byteCapacity else { return }
