...
final class AudioCodec {
    ...
    func append(_ audioBuffer: AVAudioBuffer, when: AVAudioTime) {
        ...
        var suppliedCompressedInput = false
        repeat {
            ...
            outputStatus = audioConverter.convert(to: outputBuffer, error: &error) { inNumberFrames, inputStatus in
                ...
                case is AVAudioCompressedBuffer:
                    // The packet owns its storage. Do not copy into the fixed 1024-byte
                    // scratch buffer or provide the same packet twice to the converter.
                    guard !suppliedCompressedInput,
                          let packet = audioBuffer as? AVAudioCompressedBuffer,
                          packet.byteLength > 0, packet.byteLength <= packet.byteCapacity,
                          packet.packetCount > 0, packet.packetCount <= packet.packetCapacity else {
                        inputStatus.pointee = .noDataNow
                        return nil
                    }
                    suppliedCompressedInput = true
