...
final actor MediaLink {
    ...
    private func getCurrentTime() async -> TimeInterval {
        if let audioPlayer { return await audioPlayer.currentTime }
        // macOS's default display link reports zero target-time delta. Audio-free
        // playback must use elapsed monotonic time, anchored to the first frame.
        guard let playbackStartedAt else { return 0 }
        return max(0, ProcessInfo.processInfo.systemUptime - playbackStartedAt)
    }
}
...
extension MediaLink: AsyncRunner {
    ...
    func startRunning() {
        ...
        Task {
            for await _ in displayLink.updateFrames {
